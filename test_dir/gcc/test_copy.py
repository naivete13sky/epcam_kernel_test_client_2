...
class TestGraphicEditCopy:
    ...
    def testCopy(self, job_id, prepare_test_job_clean_g):
        '''
        本用例测试Copy功能
        '''
        ...
        Layers.copy_other(job_ep, step, 'top', 'top', True, 0, 0, 0, 0, 0, 0, 0)

        # 同层复制坐标偏移
        Selection.select_feature_by_id(job_ep, step, 'l2', [840])
        Layers.copy_other(job_ep, step, 'l2', 'l2', False, 6000000, 2000000, 0, 0, 0, 0, 0)

        # 同层水平镜像复制
        Selection.select_feature_by_id(job_ep, step, 'l3', [0])
        Layers.copy_other(job_ep, step, 'l3', 'l3', False, 0, 0, 1, 0, 0, 0, 0)

        # 同层垂直镜像
        Selection.select_feature_by_id(job_ep, step, 'l4', [0])
        Layers.copy_other(job_ep, step, 'l4', 'l4', False, 0, 0, 2, 0, 0, 0, 0)

        # 旋转90度复制到新建层别
        Matrix.create_layer(job_ep, 'l5_1')
        Selection.select_feature_by_id(job_ep, step, 'l5', [0])
        Layers.copy_other(job_ep, step, 'l5', 'l5_1', False, 0, 0, 0, 0, 90, 0, 0)
