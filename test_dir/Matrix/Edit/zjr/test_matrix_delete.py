...
class TestMatrixDelete:
    ...
    共执行2个测试用例，覆盖6个测试场景
    """
    @pytest.mark.parametrize("job_id", GetTestData().get_job_id('Matrix_Delete_Step'))
    def test_matrix_delete_step(self, job_id):
        """
        id:30872,本用例测试Matrix窗口的Delete_Step功能,共执行1个测试用例，实现2个方法，覆盖3个测试场景
        """
        ...
        assert all_steps_list_job_ep == all_steps_list_job_g
        Print.print_with_delimiter("断言--结束")
        Job.close_job(job_ep)
        Job.close_job(job_g)

    @pytest.mark.parametrize("job_id", GetTestData().get_job_id('Matrix_Delete_Layer'))
    def test_matrix_delete_layer(self, job_id):
        """
        id:40757,本用例测试Matrix窗口的Delete_Layer功能,共执行1个测试用例，实现2个方法，覆盖3个测试场景
        """
        g = RunConfig.driver_g  # 拿到G软件
        test_cases = 0  # 用户统计执行了多少条测试用例
        data = {}  # 存放比对结果信息
        ...
        # ----------------------------------------开始验证结果：G与EP---------------------------------------------------------
        Print.print_with_delimiter('断言--开始')
        assert all_layers_list_job_ep == all_layers_list_job_g
        Print.print_with_delimiter("断言--结束")
        Job.close_job(job_ep)
        Job.close_job(job_g)