...
class TestGraphicSplitLayer:
    # @pytest.mark.Splitlayer
    @pytest.mark.parametrize("job_id", GetTestData().get_job_id('Split_layer'))
    def testsplit_layer(self, job_id, g, prepare_test_job_clean_g):

        '''
        本用例测试左侧菜单栏拆分层别---Split_layer，ID：35001
        '''
        ...
        # 1、根据层别属性进行拆分
        BASE.split_layer_with_attribute(job_ep,step,'l10')
        # result = json.loads(ret)['result']
        # GUI.show_layer(job_ep, step, 'l10')

        # 2、根据层别物件类型进行拆分
        BASE.split_layer_with_featuretype(job_ep, step, 'l1')
        # GUI.show_layer(job_ep, step, 'l1')

        # 3、合并拆分的层别
        BASE.split_layer_with_attribute(job_ep, step, 'l3')
        BASE.recovery_splited_layers(job_ep,step,'l3')
