...
class TestGraphicDeleteLayer:
    # @pytest.mark.DeleteLayer
    @pytest.mark.parametrize("job_id", GetTestData().get_job_id('Delete_layer'))
    def testdelete_layer(self, job_id, g, prepare_test_job_clean_g):

        '''
        本用例测试左侧菜单栏删除层别---Delete_layer，ID：35728
        '''
        ...
        # 1.验证删除layer指定层别
        BASE.delete_layer(job_ep, 18)
        # GUI.show_layer(job_ep, step, '')
