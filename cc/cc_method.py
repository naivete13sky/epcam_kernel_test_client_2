...
class DMS():
    ...
    def file_downloand(self,need_file_path, save_path):  #######文件下载
        if os.path.exists(need_file_path) == False:  # 判断是否存在文件
            ...
            # 文件url
            file_url = 'http://{}/media/files/{}'.format(RunConfig.dms_ip,os.path.basename(need_file_path))
    ...
    def get_job_fields_from_dms_db_sql(self,sql):
        pass
        conn = psycopg2.connect(database="epdms", user="readonly", password="123456", host=RunConfig.dms_ip, port="5432")
        cursor = conn.cursor()
        sql = sql
        print('sql:', sql)
        cursor.execute(sql)
